...
class LoginPage(BasePage):
    ...
    def login(self, organization=info.organization, login=info.login, password=info.password):
        ...
        #self.click_when_loaded(self.locators.ORGANIZATION_CHOICE, timeout_for_click=5, timeout_for_wait=3, retries_spinner=2, retries_no_spinner=1)
        self.key_down()
        self.send_keys(self.locators.LOGIN_FIELD, login)
        #self.click_when_loaded(self.locators.LOGIN_CHOICE, timeout_for_click=5, timeout_for_wait=3, retries_spinner=2, retries_no_spinner=1)
